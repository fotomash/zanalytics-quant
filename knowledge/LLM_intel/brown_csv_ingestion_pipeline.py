...
class BrownCSVIngestionPipeline:
    ...
    def apply_schema_binding(
        self, df: pd.DataFrame, file_metadata: Dict
    ) -> pd.DataFrame:
    ...
    def validate_confidence_thresholds(self, summary: Dict) -> Dict:
        ...
        threshold = self.confidence_matrix.get(
            pattern_type, self.confidence_matrix.get("default_threshold", 0.7)
        )
    ...
    def prepare_vector_metadata(self, summary: Dict, file_metadata: Dict) -> Dict:
        ...
        return {
            ...
            # Provide explicit fields used by clustering visualisations
            "pattern_name": summary.get("primary_pattern"),
            "pnl": summary.get("pnl"),
    ...
    def ingest_csv(self, filepath: str) -> Dict:
        ...
        try:
            ...
            for chunk_result in processing_result["summaries"]:
                if chunk_result.get("vector_ready"):
                    ...
                    vector_metadata = self.prepare_vector_metadata(
                        validated_summary, file_metadata
                    )
            ...
            result["ingestion"]["chunks_processed"] = len(
                processing_result["summaries"]
            )
