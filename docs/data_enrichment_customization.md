...
4. **Feature Generation:** `utils/enrich.py` currently augments each tick with a Wyckoff phase label, an aggregated confidence score, a human-readable nudge string, and an embedding vector. More advanced analytics are intentionally omitted for now.
...
- Introduce full technical indicators (e.g., SMA, RSI) and options Greeks as optional enrichment modules.
