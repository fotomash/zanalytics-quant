...
try:
    ...
    logger.warning("Policy loader import failed: %s", exc)
...
class RiskEnforcer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.state = {"consecutive_wins": 0, "confidence_level": 0.0}
...
class EnhancedRiskEnforcer:
    ...
    def __init__(self):
        ...
        dl_pct = risk_pol.get("daily_loss_cap_pct") or risk_pol.get("risk", {}).get(
            "max_daily_loss_pct"
        )
        pt_max = risk_pol.get("per_trade_risk_pct_max") or risk_pol.get("risk", {}).get(
            "max_single_trade_loss_pct"
        )
        max_trades = risk_pol.get("max_trades_per_day") or risk_pol.get("risk", {}).get(
            "max_daily_trades"
        )
        cooling = risk_pol.get("cool_off", {}).get("minutes")
        # Build effective config
        effective = {}
        if dl_pct is not None:
            effective["daily_loss_percent"] = (
                float(dl_pct) / 100.0 if dl_pct > 1 else float(dl_pct)
            )
        if pt_max is not None:
            effective["max_position_size"] = (
                float(pt_max) / 100.0 if pt_max > 1 else float(pt_max)
            )
    ...
    def get_risk_status(self) -> Dict:
        ...
        if ds.get("consecutive_losses", 0) >= self.core.limits.get(
            "max_consecutive_losses", 3
        ):
        ...
        return {
            ...
            "trades_remaining": max(
                0,
                int(self.core.limits.get("max_trades_per_day", 5))
                - int(ds.get("trades_count", 0)),
            ),
            "status": status,
            "warnings": warnings,
    ...
    def allow(self, signal: Dict) -> Tuple[bool, List[str], Dict]:
        ...
        details = {
            ...
            "loss_buffer": self.limits["daily_loss_limit"]
            - abs(self.daily_stats["total_pnl"]),
    ...
    def _check_trade_count(self) -> Dict:
        ...
        if self.daily_stats["trades_count"] == self.limits["max_trades_per_day"] - 1:
            return {"allowed": True, "warning": "⚠️ This is your last trade for today"}
    ...
    def _check_cooling_period(self) -> Dict:
        ...
        if self.daily_stats["cooling_until"]:
            if datetime.now() < self.daily_stats["cooling_until"]:
                remaining = (
                    self.daily_stats["cooling_until"] - datetime.now()
                ).seconds // 60
                return {
                    "allowed": False,
    ...
    def _check_behavioral_patterns(self) -> Dict:
        ...
        if self.daily_stats["consecutive_losses"] >= 2:
            warnings.append(
                f'📉 {self.daily_stats["consecutive_losses"]} consecutive losses - consider a break'
            )
    ...
    def _check_overconfidence(self) -> Dict:
        ...
        if (
            self.state.get("consecutive_wins", 0) >= 3
            or self.state.get("confidence_level", 0) > 0.8
        ):
            return {
                "detected": True,
    ...
    def get_behavioral_report(self) -> Dict:
        ...
        return {"current_state": state, "recommendations": recommendations}
    ...
    def update_trade_result(self, result: Dict):
        ...
        if result.get("pnl", 0) < 0:
            ...
            if abs(result.get("pnl", 0)) > 100:
                self.daily_stats["cooling_until"] = datetime.now() + timedelta(
                    minutes=self.limits["cooling_period_minutes"]
                )
        else:
