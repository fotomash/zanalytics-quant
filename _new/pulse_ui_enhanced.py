...
class PulseUIEnhanced:
    ...
    def render_real_performance_metrics(self):
        """
        Display REAL performance metrics from MT5, not mock data.
        """
        ...
        with col1:
            ...
            st.metric(
                "Real Win Rate",
                f"{win_rate:.1f}%",
                delta=f"{win_rate - 50:.1f}% vs baseline"
            )
        ...
        with col2:
            ...
            st.metric(
                "Total P&L",
                f"${total_pnl:,.2f}",
                delta=f"{(total_pnl/10000)*100:.1f}% of account"
            )
        ...
        with col3:
            ...
            st.metric(
                f"{color} Revenge Trades",
                revenge_count,
                delta="Need discipline" if revenge_count > 2 else "Good control"
            )
        ...
        with col4:
            ...
            st.metric(
                ...
                delta="High risk" if avg_risk_score > 5 else "Controlled"
            )

    def render_behavioral_heatmap(self):
        """
        Show when you actually trade well vs poorly.
        This is self-knowledge through data.
        """
        df = self.bridge.get_real_trade_history()
    ...
    def render_confluence_validation(self):
        """
        Prove that higher confluence = better outcomes.
        This validates the entire scoring system.
        """
        ...
        if len(validation) > 0:
            ...
            if high_confluence_wr > low_confluence_wr:
                st.success(f"✅ Validation confirmed: High confluence (80-90) has {high_confluence_wr:.1f}% win rate vs {low_confluence_wr:.1f}% for low confluence")
            else:
                st.warning("⚠️ Confluence scoring needs calibration")

    def render_weekly_review(self):
        """
        The weekly mirror - see yourself as you really trade.
        """
        ...
        with col3:
            ...
            total_incidents = (insights['revenge_trading_incidents'] + 
    ...
    def render_decision_surface(self):
        ...
        with col1:
            # Real confluence score from kernel
        ...
        # Top opportunities (curated, not overwhelming)
...
def integrate_real_data():
    """
    Function to integrate into existing pulse_dashboard.py
    """
