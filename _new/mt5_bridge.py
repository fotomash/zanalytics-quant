...
class MT5Bridge:
    ...
    def get_real_trade_history(self, days_back: int = 30) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with actual trades, not mock data
        """
    ...
    def _enrich_with_behavioral_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        This is where the magic happens - turning raw trades into behavioral insights.
        """
        ...
        df['consecutive_wins'] = df.groupby((df['is_win'] != df['is_win'].shift()).cumsum())['is_win'].cumsum()
        df['consecutive_losses'] = df.groupby((~df['is_win'] != ~df['is_win'].shift()).cumsum()['is_win'].apply(lambda x: (~x).cumsum())
        ...
        # Detect overconfidence (increased position after wins)
        df['position_change'] = df['volume'].pct_change()
        df['overconfidence'] = (df['consecutive_wins'] >= 3) & (df['position_change'] > 0.5)

        # Time-based patterns
        df['hour'] = df['time'].dt.hour
        df['is_optimal_time'] = df['hour'].between(8, 20)  # London/NY sessions
        df['fatigue_trade'] = df.groupby(df['time'].dt.date).cumcount() > 5  # More than 5 trades/day

        # FOMO detection (chasing after missing moves)
        df['entry_speed'] = df['time_since_last'].dt.total_seconds()
        df['fomo_trade'] = df['entry_speed'] < 60  # Entry within 1 minute of last trade

        # Calculate running P&L for the day
        df['daily_pnl'] = df.groupby(df['time'].dt.date)['profit'].cumsum()
        df['daily_drawdown'] = df.groupby(df['time'].dt.date)['daily_pnl'].cummin()

        # Risk score based on behavioral flags
    ...
    def get_behavioral_report(self) -> Dict:
        ...
        This is what makes the dashboard powerful - real patterns, not theory.
        """
        ...
        report = {
            "performance_metrics": {
                ...
                "profit_factor": abs(df[df['is_win']]['profit'].sum() / df[~df['is_win']]['profit'].sum()),
            ...
            "behavioral_patterns": {
                ...
                "fatigue_trades": {
                    "count": df['fatigue_trade'].sum(),
                    "success_rate": df[df['fatigue_trade']]['is_win'].mean() * 100 if df['fatigue_trade'].any() else 0,
                    "typical_time": df[df['fatigue_trade']]['hour'].mode().values[0] if df['fatigue_trade'].any() else None
                },
                "fomo_trades": {
                    "count": df['fomo_trade'].sum(),
            ...
            "risk_violations": {
                "overtrade_days": len(df.groupby(df['time'].dt.date).size()[df.groupby(df['time'].dt.date).size() > 5]),
                "max_daily_trades": df.groupby(df['time'].dt.date).size().max(),
    ...
    def get_confluence_validation(self) -> pd.DataFrame:
        ...
        This proves whether the scoring system actually works.
        """
        df = self.get_real_trade_history()
        ...
        # Simulate confluence scores (in production, these would be stored)
        # This is where you'd pull actual scores from your signal journal
        df['confluence_score'] = np.random.randint(40, 95, size=len(df))

        # Bin confluence scores
        df['confluence_band'] = pd.cut(df['confluence_score'], 
    ...
    def sync_to_pulse_journal(self, journal_path: str = "signal_journal.json"):
        ...
        This creates the accountability loop.
        """
        df = self.get_real_trade_history()
        ...
        for _, trade in df.iterrows():
            ...
            if not any(e.get('data', {}).get('ticket') == entry['data']['ticket'] for e in journal):
    ...
    def generate_weekly_review(self) -> Dict:
        ...
        This is the reflection engine - turning data into wisdom.
        """
        ...
        review = {
            ...
            "summary": {
                ...
                "best_day": df.groupby(df['time'].dt.date)['profit'].sum().idxmax().strftime("%Y-%m-%d"),
                "worst_day": df.groupby(df['time'].dt.date)['profit'].sum().idxmin().strftime("%Y-%m-%d")
            },
            "behavioral_insights": {
                "revenge_trading_incidents": df['revenge_trade'].sum(),
                "overconfidence_incidents": df['overconfidence'].sum(),
                "fatigue_incidents": df['fatigue_trade'].sum(),
        ...
        if df['revenge_trade'].sum() > 2:
            review["recommendations"].append("Implement mandatory 30-min cooldown after losses")

        if df['overconfidence'].sum() > 3:
            review["recommendations"].append("Reduce position size by 50% after 3 consecutive wins")

        if df['fatigue_trade'].sum() > 5:
            review["recommendations"].append("Limit daily trades to 5 maximum")

        if df[~df['is_optimal_time']].sum() > len(df) * 0.3:
            review["recommendations"].append("Focus trading on London/NY session hours only")
...
if __name__ == "__main__":
    # Initialize bridge (replace with your MT5 credentials)
    bridge = MT5Bridge()

    # For demo purposes, create sample integration
