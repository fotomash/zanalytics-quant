...
try:
    ...
    bhv_score = behavioral_score_from_mirror(mirror)  # 0..100 composite
...
def _render_unified_whisperer_block(account_info: Dict, risk_env: Dict):
    ...
    # Session vitals are already shown at the top of the page
...
def main():
    ...
    # Sidebar: Sources (status) similar to 07
    with st.sidebar.expander("Sources (status)", expanded=False):
        base = (st.session_state.get('adv19_api_base') or os.getenv('DJANGO_API_URL', 'http://django:8000')).rstrip('/')
        try:
            ok_market = requests.get(f"{base}/api/v1/market/mini", timeout=1.0).ok
        except Exception:
            ok_market = False
        try:
            ok_mirror = requests.get(f"{base}/api/v1/mirror/state", timeout=1.0).ok
        except Exception:
            ok_mirror = False
        try:
            ok_equity = requests.get(f"{base}/api/v1/feed/equity/series", timeout=1.0).ok
        except Exception:
            ok_equity = False
        mt5b = getattr(pulse_manager, 'mt5_url', None)
        try:
            ok_bridge = requests.get(f"{str(mt5b).rstrip('/')}/account_info", timeout=1.0).ok if mt5b else False
        except Exception:
            ok_bridge = False
        st.caption(f"Django API: {base}")
