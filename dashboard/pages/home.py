...
def get_enriched_bars(symbol: str, count: int = 5000, tf: str = "1min") -> pd.DataFrame:
    ...
    try:
        ...
        # Price: last or mid
        if "last" in df:
            df["price"] = df["last"].fillna(
                (df["bid"] + df["ask"]) / 2 if "bid" in df and "ask" in df else np.nan
            )
    ...
    bars["vol_z"] = (
        bars["volume"] - bars["volume"].rolling(win).mean()
    ) / bars["volume"].rolling(win).std().fillna(1)
    ...
    if redis_client:
        redis_client.set(
            cache_key(symbol, tf), bars.to_json(orient="split"), ex=300
        )  # 5min TTL
    if db_engine:
        try:
...
with st.sidebar:
    ...
    st.markdown("---")
    max_loss_pct = st.slider("Max Loss %", 0.1, 3.0, 2.0)
    anticipated_trades = st.slider("Anticipated Trades", 1, 12, 10)
...
# Risk metrics
todays_loss = enriched["ret"].sum() * 100 if not enriched.empty else 0.0
max_allowed_loss = max_loss_pct
per_trade_loss = max_allowed_loss / anticipated_trades
