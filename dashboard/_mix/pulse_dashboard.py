...
def run_backtest_service() -> dict:
    ...
    data = pd.DataFrame(
        {"close": prices},
        index=pd.date_range(end=datetime.now(), periods=len(prices), freq="H"),
    )
...
# Header
st.markdown(
    '<div class="pulse-header">🧠 ZANALYTICS PULSE</div>', unsafe_allow_html=True
)
st.markdown(
...
with col1:
    confluence_color = (
        "#00ff44"
        if st.session_state.confluence_score >= 70
        else "#ffaa00" if st.session_state.confluence_score >= 50 else "#ff4444"
    )
    st.metric(
        "Confluence Score",
...
with tab1:
    ...
    with col2:
        ...
        max_risk = st.select_slider(
            "Max Risk Level", options=["LOW", "MEDIUM", "HIGH"], value="MEDIUM"
        )
...
with tab2:
    ...
    with col1:
        ...
        fig_pnl.update_layout(
            title="30-Day P&L Curve", height=300, template="plotly_dark"
        )
    ...
    with col3:
        ...
        avg_rr = (
            abs(backtest_metrics["avg_win"] / backtest_metrics["avg_loss"])
            if backtest_metrics["avg_loss"]
            else float("inf")
        )
