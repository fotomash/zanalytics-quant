...
class UltimateIndicatorEngine:
    ...
    def _add_quick_enrichment(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            df["session"] = df.index.tz_convert("UTC").hour.map(
        ...
        try:
            if "bid" in df.columns and "ask" in df.columns:
                ...
                analysis = {
                    ...
                    "autocorrelation": (
                        np.corrcoef(price_changes[:-1], price_changes[1:])[0, 1]
        ...
        try:
            if "bid" in df.columns and "ask" in df.columns:
                ...
                analysis = {
                    ...
                    "wide_spread_count": np.sum(
                        spread > np.mean(spread) + 2 * np.std(spread)
                    ),
                    "narrow_spread_count": np.sum(
                        spread < np.mean(spread) - np.std(spread)
                    ),
                }
        ...
        try:
            if "volume" in df.columns:
                ...
                analysis = {
                    ...
                    "high_volume_ticks": np.sum(
                        volume > np.mean(volume) + 2 * np.std(volume)
                    ),
                    "volume_percentiles": {
    ...
    def _analyze_order_flow(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            if "bid" in df.columns and "ask" in df.columns and "last" in df.columns:
                ...
                buy_volume = sum(
                    volume[i] for i, t in enumerate(trade_classification) if t == "buy"
                )
                sell_volume = sum(
                    volume[i] for i, t in enumerate(trade_classification) if t == "sell"
                )
                ...
                analysis = {
                    ...
                    "aggressive_buy_ratio": np.sum(
                        [1 for t in trade_classification if t == "buy"]
                    )
                    / len(trade_classification),
                    "aggressive_sell_ratio": np.sum(
                        [1 for t in trade_classification if t == "sell"]
                    )
                    / len(trade_classification),
                }
...
class UltimateDataProcessor:
    ...
    async def process_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            try:
                ...
                parquet_root = (
                    os.path.join(base_output_dir, "parquet")
                    if getattr(self.config, "export_parquet", True)
                    else None
                )
                json_root = (
                    os.path.join(base_output_dir, "json")
                    if getattr(self.config, "export_json", False)
                    else None
                )
                csv_root = (
                    os.path.join(base_output_dir, "csv")
                    if getattr(self.config, "export_csv", True)
                    else None
                )
                ...
                if parquet_root or json_root or csv_root:
                    ...
                    try:
                        save_results_to_parquet_and_json(
                            symbol, data_by_timeframe, pr, jr, cr
                        )
                        logger.info(
                            "Saved outputs for %s → parquet:%s json:%s csv:%s " "(%s)",
                            symbol,
                            bool(pr),
                            bool(jr),
                            bool(cr),
    ...
    async def _load_data(self, file_path: str) -> Optional[pd.DataFrame]:
        """
        Robustly load data from CSV and JSON files, handling datetime indices and avoiding missing/None timestamps.
        """
        try:
            if file_path.endswith(".csv"):
                df = pd.read_csv(file_path, sep=",")
                # If 'time' column exists (MT5 export), parse and set as index
                ...
                # If 'timestamp' already exists, parse and set as index
                elif "timestamp" in df.columns:
                    df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
                    df.set_index("timestamp", inplace=True)
                # If index is not datetime, attempt to parse it
...
def save_results_to_parquet_and_json(
    symbol, data_by_timeframe, parquet_root=None, json_root=None, csv_root=None
):
    """
    ...
    for tf, df in data_by_timeframe.items():
        ...
        if isinstance(df_export.index, pd.DatetimeIndex):
            ...
            df_export["timestamp"] = pd.to_datetime(
                df_export["timestamp"], errors="coerce"
            )
        else:
            ...
            if time_col:
                df_export["timestamp"] = pd.to_datetime(
                    df_export[time_col], errors="coerce"
                )
            else:
                import warnings

                warnings.warn(
                    f"No datetime index or time column found for {symbol} {tf}. Filling NaT."
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="ncOS Ultimate Microstructure Analyzer - ALL FEATURES ENABLED"
    )
    ...
    if args.dry_save_test:
        test_dirs = [Path(args.output_dir) / sub for sub in ("parquet", "json", "csv")]
        for d in test_dirs:
            try:
    ...
    if args.file:
        ...
        if directory.exists():
            ...
            csv_pattern = (
                args.pattern.replace("*", "*.csv")
                if not args.pattern.endswith(".csv")
                else args.pattern
            )
            json_pattern = (
                args.pattern.replace("*", "*.json")
                if not args.pattern.endswith(".json")
                else args.pattern
            )
    ...
    logger.info(
        f"🚀 ULTIMATE ANALYSIS STARTING - Processing {len(files_to_process)} files with ALL FEATURES ENABLED"
    )
    ...
    logger.info(
        f"🎉 ULTIMATE ANALYSIS COMPLETE! Processed {len(files_to_process)} files"
    )
    logger.info(
...
def _log_ml_summary(metrics, logger=logger):
    ...
    logger.info(
        f"ML Metrics: MSE={metrics.get('mse')}, RMSE={metrics.get('rmse')}, R2={metrics.get('r2')}"
    )
