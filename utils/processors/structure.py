...
class StructureProcessor:
        ...
        try:
            if {'bid', 'ask', 'volume'} <= set(df.columns):
                ...
                for i in range(1, len(df)):
                    ...
                    if ((bid_change > np.std(bid_diff) * 3 or
                         ask_change > np.std(ask_diff) * 3) and volume[i] == 0):
                        events.append({
                            'index': int(i),
                ...
                for i in range(10, len(df)):
                    ...
                    if (recent_volume > np.mean(volume) * 5 and
                        ...
                        events.append({
                            ...
                            'suspicion_level': float(recent_volume / (price_range + 1e-10))
                        })
        except Exception as e:  # pragma: no cover - defensive logging
    ...
    def _detect_quote_stuffing(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(timestamps) > 1:
                times = [t.timestamp() if hasattr(t, 'timestamp') else t for t in timestamps]
        ...
        for i in range(50, len(df) - 10):
            try:
                if vol_20[i] < 0.75 * vol_50m[i] and volume[i] > 1.25 * volu_20[i]:
                    phases.append(dict(phase='accumulation', start=i - 10, end=i + 10,
                                       strength=volume[i] / volu_20[i]))
                elif (vol_20[i] > 1.25 * vol_50m[i]
                      and close[i] > 1.01 * close[i - 5]
                      and volume[i] > volu_20[i]):
                    phases.append(dict(phase='markup', start=i - 5, end=i + 5,
        ...
        try:
            ...
            for i in range(10, len(df) - 10):
                if (close[i] < close[i - 5] and volume[i] > np.mean(volume[i - 10:i]) and
                        close[i + 1] > close[i]):
                    events.append({'event': 'PS', 'index': int(i), 'price': float(close[i]),
                                   'volume': float(volume[i]),
                                   'description': 'Preliminary Support'})
            for i in range(10, len(df) - 10):
                if (volume[i] > 2 * np.mean(volume[i - 10:i]) and
                        close[i] < close[i - 1] and close[i + 1] > close[i]):
                    events.append({'event': 'SC', 'index': int(i), 'price': float(close[i]),
                                   'volume': float(volume[i]),
    ...
    def _analyze_volume_patterns(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                'divergence': bool((price_trend > 0 and volume_trend < 0) or (price_trend < 0 and volume_trend > 0)),
